...
mod tests {
    ...
    fn simple_ode() {
        ...
        unsafe {
            ...
            assert!(
                SUNContext_Create(comm_no_mpi(), &raw mut ctx) >= 0,
            ...
            *N_VGetArrayPointer(y) = 1.0;
            ...
            let mut t = 0f64;
            CVode(cvode_mem, 1.0, y, &raw mut t, CV_NORMAL);
            // y[0] is now exp(-1)

            let result = (*N_VGetArrayPointer(y) * 1e6) as i32;
            assert_eq!(result, 367_879);

            N_VDestroy(y);
            CVodeFree(&raw mut cvode_mem);
